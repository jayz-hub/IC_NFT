...
// import Utils "utils";
shared(msg) actor class ERC721(creater : Principal){
    private stable var _owner : Principal = creater;
    private stable var _createTime : Time.Time = Time.now();
    
    type TokenId = Types.TokenId;
    type TokenMeta = Types.TokenMeta;
    private var nfts = HashMap.HashMap<TokenId,TokenMeta>(1,Types.equal,Types.hash);
    ///mapping from nft to owner 
    private var nftToOwner = HashMap.HashMap<TokenId,Principal>(1,Types.equal,Types.hash);
    ...
    public query(msg) func getMetadata(tokenId : TokenId) : ?TokenMeta{
        nfts.get(tokenId);
    };
